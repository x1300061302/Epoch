...
CONTAINS
    !Xiey annihilation
  SUBROUTINE particle_annihilation
    !Some Steps: refer to the collision.F90
    !1,judege if the e+ and e- annihilation
    !2,calculate the momentum and energy of generated photon 
    !3,remove e+ and e- and generate photon

    Do ispecies = 1, n_species
      IF (species_list(ispecies)%species_type /= c_species_id_electron) &
          CYCLE
      !only for electron 
      annihilation_species = .FALSE.
      DO jspecies = ispecies+1, n_species
        IF (species_list(jspecies)%species_type == c_species_id_positron) THEN
          annihilation_species = .TRUE.
          EXIT
        ENDIF
      ENDDO
      !Now ispecies is electron and jspecies is positron

      IF (.NOT.annihilation_species) CYCLE
...
!
  ...
  END SUBROUTINE particle_annihilation

  SUBROUTINE annihilation(p_list1,p_list2)
      !1,Initial Assumption electron and positron must annihilation when they encounter each other 
      !2,Simple Assumption, for quasi-particle.
    TYPE(particle_list), INTENT(INOUT) :: p_list1
    TYPE(particle_list), INTENT(INOUT) :: p_list2

    TYPE(particle), POINTER :: electron, positron

    TYPE(particle), POINTER :: photon1, photon2

    INTEGER(i8) :: icount, jcount, pcount, k


    !The number of p_list  
    icount = p_list1%count
    jcount = p_list2%count
    ! Different from collision, This is min
    pcount = MIN(icount, jcount)
    ! annihilation e+ and e- generate photon

    electron => p_list1%head
    positron => p_list2%head
    Do k = 1, pcount 
        !refer to generate_pair in photon.F90
        CALL create_particle(photon1)
        CALL create_particle(photon2)

        ! Asumption the pos is same;
        photon1%part_pos = electron%part_pos
        photon2%part_pos = electron%part_pos

        !Asumption two binary collision refer to collision.F90
        p1 = electron%part_p
        p2 = positron%part_p
        ...
        !theta = (arcsin(e1 + e2)/sqrt(2 c p_norm)) - pi/4

        photon1%part_pos = p1
        photon2%part_pos = p2
        !After Calculate the momentum of photon 


        !remove e+ and e-
        CALL remove_particle_from_partlist(species_list(ielectron)%attached_list,electron)
        CALL remove_particle_from_partlist(species_list(ipositron)%attached_list,positron)
